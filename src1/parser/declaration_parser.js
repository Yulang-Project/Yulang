...
import { Stmt, LetStmt, FunctionDeclaration, ClassDeclaration, StructDeclaration, PropertyDeclaration, DeclareFunction, ImportStmt, ConstStmt, Parameter, TypeAnnotation, Expr, ExpressionStmt, LiteralExpr, AsExpr, ObjectLiteralExpr, AddressOfExpr } from '../ast.js';
...
export class DeclarationParser {
    ...
    functionDeclaration(kind, isExported, visibility = new Token(TokenType.PUBLIC, 'public', null, 0, 0), isStatic = false) {
    ...
    letDeclarationForForLoop(isExported) {
    ...
    importDeclaration() {
        ...
        const modulePath = pathToken.literal;
        // Resolve module path using the finder
        let fullModulePath;
        if (modulePath === 'std' || modulePath.startsWith('std/')) {
            fullModulePath = this.parser.finder.getStdLibModulePath(this.parser.osIdentifier, this.parser.archIdentifier, modulePath);
        }
...
//# sourceMappingURL=declaration_parser.js.map