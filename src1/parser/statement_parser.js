...
import { Stmt, ExpressionStmt, IfStmt, WhileStmt, ReturnStmt, BlockStmt, LetStmt, Expr, LiteralExpr, FunctionLiteralExpr, FunctionTypeAnnotation, TypeAnnotation, BasicTypeAnnotation // Added new AST imports
 } from '../ast.js';
...
export class StatementParser {
    ...
    statement() {
        ...
        if (this.parser.match(TokenType.WHILE))
            return this.whileStatement();
        if (this.parser.match(TokenType.FOR))
            return this.forStatement(); // NEW: Handle for loop
        if (this.parser.match(TokenType.RETURN))
        ...
        if (this.parser.match(TokenType.FUN)) { // Handle local function declarations (desugar into let name = fun(...) { ... };)
            ...
            const funcTypeAnnotation = new FunctionTypeAnnotation(paramTypesForFuncType, returnTypeForFuncType);
