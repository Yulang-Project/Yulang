...
export class ProjectFinder {
    projectRoot; // 项目根目录
    stdLibRootPath; // 标准库在项目根目录下的相对路径，例如 "libs"
    constructor(projectRoot, stdLibRootPath = 'libs') {
        this.projectRoot = projectRoot;
        this.stdLibRootPath = stdLibRootPath;
    }
    // 获取标准库模块的完整路径 (e.g., "std/io" -> "/path/to/project_root/libs/linux/x86_64/std/io.yu")
    getStdLibModulePath(osIdentifier, archIdentifier, moduleName) {
    ...
    getBootstrapPath(osIdentifier, archIdentifier) {
    ...
    // 获取动态链接器路径
    getLinkerDynamicLinker(osIdentifier, archIdentifier) {
    ...
    // 获取额外的链接器标志
    getLinkerFlags(osIdentifier, archIdentifier) {
