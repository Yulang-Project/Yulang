...
export function emitSyscallInlineASM_X86_64(generator, callNum, args) {
    ...
    for (let i = 0; i < args.length && i < argRegisters.length; i++) {
        inputConstraints.push(argRegisters[i]);
    }
...
export function emitSyscall_X86_64(platform, generator, callNum, args) {
    const syscallArgs = args.map(a => {
        if (a.type.endsWith('*')) {
...
//# sourceMappingURL=syscalls.js.map